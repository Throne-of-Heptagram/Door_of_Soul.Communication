...
namespace Door_of_Soul.Communication.HexagramEntranceServer
{
    public abstract class HistoryCommunicationService : HexagramCommunicationService
    {
        ...
        public bool HandleEvent(HistoryEventCode eventCode, Dictionary<byte, object> parameters, out string errorMessage)
        {
            return HistoryEventRouter.Instance.Route(eventCode, parameters, out errorMessage);
        }
        public bool HandleOperationResponse(HistoryOperationCode operationCode, OperationReturnCode returnCode, string operationMessage, Dictionary<byte, object> parameters, out string errorMessage)
        {
